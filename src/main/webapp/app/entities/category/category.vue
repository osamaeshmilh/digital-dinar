<template>
  <div>
    <h2 id="page-heading" data-cy="CategoryHeading">
      <span v-text="t$('digitalDinarApp.category.home.title')" id="category-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('digitalDinarApp.category.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'CategoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('digitalDinarApp.category.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && categories && categories.length === 0">
      <span v-text="t$('digitalDinarApp.category.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="categories">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('nameAr')">
              <span v-text="t$('digitalDinarApp.category.nameAr')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('nameEn')">
              <span v-text="t$('digitalDinarApp.category.nameEn')"></span>
            ...
            <th scope="row" v-on:click="changeOrder('code')">
              <span v-text="t$('digitalDinarApp.category.code')"></span>
        ...
        <tbody>
          <tr v-for="category in categories" :key="category.id" data-cy="entityTable">
            ...
            <td>{{ formatDateShort(category.createdDate) || '' }}</td>
            <td>{{ category.lastModifiedBy }}</td>
            <td>{{ formatDateShort(category.lastModifiedDate) || '' }}</td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="digitalDinarApp.category.delete.question" data-cy="categoryDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-category-heading" v-text="t$('digitalDinarApp.category.delete.question', { id: removeId })"></p>
      </div>
